...
### Broadcast Graphics (lower thirds, ticker, logo bug)
...
- **Implementation runtime:** Graphics are rendered natively with **`@babylonjs/gui`
  ...
  overlay attached to the live scene. On-air graphics are part of the actual rendered
  frame — they show up in captured thumbnails and any output stream, not a DOM mock.
...
- Built natively on the existing Babylon engine (Apache-2.0). Captures real node
