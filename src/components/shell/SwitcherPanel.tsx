...
import { Video, Plus, Trash2, ArrowRight, Film, Image as ImageIcon, Monitor as MonitorIcon } from 'lucide-react';
...
export function SwitcherPanel() {
  const { sources, addWebcamSource, addVideoFileSource, addImageFileSource, addScreenSource, removeSource, setPreview, cut, previewSource, programSource, roleOf, previewId, updateSourcePlacement, updateSourceKeying } =
    useSources();
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <Button variant="secondary" onClick={() => void addWebcamSource()} title="Add webcam or camera input">
              <Plus size={14} /> Webcam
            </Button>
            <Button variant="secondary" onClick={() => videoFileRef.current?.click()} title="Add video file (MP4, WebM, MOV)">
              <Film size={14} /> Video
            </Button>
            <input
              ref={videoFileRef}
              ...
              aria-label="Add video file"
            />
            <Button variant="secondary" onClick={() => imageFileRef.current?.click()} title="Add static image (PNG, JPEG)">
            ...
            />
            <Button variant="secondary" onClick={() => void addScreenSource()} title="Add screen or window capture">
              <MonitorIcon size={14} /> Screen
            </Button>
          </div>
        </div>
        ...
        {sources.length === 0 ? (
          ...
          >
            No sources yet. Click <strong>Webcam</strong>, <strong>Video</strong>, <strong>Image</strong>, or <strong>Screen</strong> to add your first source.
          </div>
        ) : (
