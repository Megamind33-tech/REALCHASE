...
const out='docs/evidence/phase-2/keying-garbage'; fs.mkdirSync(out,{recursive:true});
...
// keep most of the frame, then crop with the garbage matte
await rng('Similarity',0.03); await rng('Crop left',0.12); await rng('Crop right',0.62); await rng('Crop top',0.1); await rng('Crop bottom',0.66); await sleep(300);
await page.screenshot({path:`${out}/01-garbage-controls.png`});
...
await vp('02-garbage-matte-cropped.png');
fs.writeFileSync(`${out}/garbage.txt`,['crop L0.12 R0.62 T0.1 B0.66 (matte should be white only inside that rect)','','--- console ---',...logs].join('\n'));
await browser.close(); console.log('garbage evidence ->',out);
